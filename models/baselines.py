...
class GaussianKernelJacobian:
    """
    [Baseline 1: Continuous Spatial Statistics Paradigm]
    Mechanism: Continuous Gaussian kernel smoothing -> Locating velocity roots -> Least-squares Jacobian matrix estimation.
    Vulnerability: Structurally confounded by metric leakage. Forced spatial derivatives exponentially amplify 
    high-frequency noise, triggering massive false positive artifacts (Derivative Amplification) and consistently 
    violating global Poincaré-Hopf constraints.
    """
    ...
    def fit(self, coords, V, normals, simplices):
        ...
        # Continuous Gaussian Kernel Smoothing
        for i in range(N):
        ...
        # Root finding (The "Zero-Speed Paradox" occurs here if vectors are strictly phase-normalized)
        ...
        for c_idx in candidates:
            ...
            try:
                # Least Squares Jacobian Estimation
...
class MarkovRandomWalk:
    """
    [Baseline 2: Graph Machine Learning Paradigm (Graph ML)]
    Mechanism: Transition probability matrix construction -> Markov steady-state random walks to identify accumulation hubs.
    Vulnerability: Structurally saddle-blind. Due to physical probability conservation, diffusing Markov particles 
    inherently roll off saddle points and pool at boundaries. Mathematically incapable of discovering 
    unstable topological bifurcations (-1 charges).
    """
    ...
    def fit(self, coords, V, normals, simplices):
        ...
        # Notice that Graph ML is structurally blind to -1 charges (Bifurcations)
